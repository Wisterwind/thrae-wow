...
function TinyTipIcons_ShowBuffs(unit)
	...
	for i = 1,num,1 do
			_,_,tmp,tmp2 = _G.UnitBuff(unit, i, flag ~= 1)
			if tmp then
				count = count + 1
				if flag ~= 3 then
...
function TinyTipIcons_ShowDebuffs(unit)
	...
	if flag >= 4 then
		...
		for i,_ in pairs(TinyTipExtrasLocale_DebuffMap) do
			dtable[i] = 0
		end
	end
	if flag == 1 then num = 8 
	...
	for i = 1,num,1 do
		if flag >= 4 and _G.UnitIsFriend(unit,"player") then
			_,_,_,_,tmp = _G.UnitDebuff(unit,i,flag ~= 5)
